...
impl Vault {
    ...
    /// Process redemption against this vault
    pub fn process_redemption(
        ...
        // Reduce debt
        self.debts.remove_debt(currency.clone(), stable_amount)?;
        
        // Reduce collateral
        if collateral_amount > self.collateral_btc {
            return Err(BitStableError::InsufficientCollateral {
                required: collateral_amount.to_btc(),
                provided: self.collateral_btc.to_btc(),
            });
        }
        
        self.collateral_btc -= collateral_amount;
        
        log::info!(
            "Processed redemption on vault {}: -{} collateral, -{} {} debt",
            self.id,
...
impl VaultManager {
    ...
    pub fn get_active_vaults(&self) -> Vec<&Vault> {
        self.vaults
            .values()
            .filter(|vault| matches!(vault.state, VaultState::Active))
            .collect()
    }
    
    /// Process redemption for a vault
    pub fn process_redemption(
        &mut self,
        vault_id: Txid,
        currency: Currency,
        ...
        // Reduce debt by redemption amount
        vault.debts.remove_debt(currency.clone(), amount)?;
        
        log::info!("Processed redemption: {} {} from vault {} for {}", 
                  amount, currency.to_string(), vault_id, redeemer);
