...
use crate::governance::{GovernanceSystem, Keyholder, KeyholderRole};
...
/// Key update tracking for governance-controlled rotations
#[derive(Debug, Clone, Serialize, Deserialize)]
...
impl CustodyManager {
    pub fn new(config: &ProtocolConfig) -> Result<Self> {
        ...
        // Initialize governance system with geographic distribution
        let mut governance_system = GovernanceSystem::new();
        Self::initialize_keyholders(&mut governance_system, &protocol_keys)?;
    ...
    /// Initialize keyholders with geographic distribution
    fn initialize_keyholders(
        ...
        for (i, key) in protocol_keys.iter().enumerate() {
            let keyholder = Keyholder {
                pubkey: *key,
                weight: 1.0,
                role: if i < 2 { KeyholderRole::Core } else { KeyholderRole::Technical },
                geographic_region: regions[i % regions.len()].to_string(),
                institution: Some(format!("Institution_{}", i)),
                added_at: Utc::now(),
                last_activity: Utc::now(),
                emergency_powers: i < 3, // First 3 have emergency powers
            };
            
            governance_system.add_keyholder(keyholder)?;
        }
